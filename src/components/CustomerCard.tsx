...
export default function IssueCard({ issue, assembly, onResolve }: Props) {
  ...
  return (
    <div className={`bg-white rounded-xl border-2 p-4 shadow-sm transition-all ${
      isMatch ? "border-blue-400 shadow-blue-100" : "border-gray-200"
    }`}>
      {/* Issue header */}
      <div className="flex items-start gap-2 mb-2">
        <span className="text-2xl">{issue.emoji}</span>
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-gray-800 text-sm leading-tight">{issue.title}</div>
          ...
          <div className="w-full h-1.5 bg-gray-200 rounded-full mt-1.5">
            <div
              className={`h-1.5 rounded-full transition-all ${patienceColor}`}
              style={{ width: `${patiencePercent}%` }}
            />
          </div>
        </div>
        <div className="text-xs font-bold text-blue-600 whitespace-nowrap">+{issue.reward}%</div>
      </div>
        ...
        <div className="flex gap-1 flex-wrap">
          {issue.requiredResources.map((resId, i) => {
            ...
            return (
              <span
                key={i}
                className={`text-base px-1.5 py-0.5 rounded text-center transition-all ${res.color} ${
                  isPlaced ? (isCurrentMatch ? "ring-2 ring-green-400" : "ring-2 ring-red-400") : "opacity-60"
                }`}
                title={res.label}
              >
                {res.emoji}
